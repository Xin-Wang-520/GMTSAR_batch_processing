...
Mode 1:
  Start sbas_burst_pin/run_sbas_parallel.sh through nohup.
  Do not add nohup or a trailing & yourself.

Monitor:
  tail -f sbas_burst_pin/run4.3_sbas_parallel.log

Check PID:
  ps -fp $(cat sbas_burst_pin/run4.3_sbas_parallel.pid)
...
if (( $# == 0 )); then
    ...
    printf '%s\n' '[NEXT] ./run4.3_sbas_parallel_pin_burst.sh 1'
    exit 0
fi
