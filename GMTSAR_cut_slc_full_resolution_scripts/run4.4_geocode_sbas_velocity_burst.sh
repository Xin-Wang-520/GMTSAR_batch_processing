...
Usage:
  ...
  ./run4.4_geocode_sbas_velocity_burst.sh 1 [CPT_MIN CPT_MAX [CPT_STEP]]
...
Mode 1:
  Project sbas_burst/vel.grd to geographic coordinates and create PDF/KML.

Default settings:
  Spatial filter: automatically detected from gauss_<meters>
  Color range  : automatically read from the generated vel_ll.grd
  Example fixed : ./run4.4_geocode_sbas_velocity_burst.sh 1 -20 20
  With step    : ./run4.4_geocode_sbas_velocity_burst.sh 1 -20 20 2
...
if (( $# == 0 )); then
    ...
    printf '%s\n' '[NEXT] ./run4.4_geocode_sbas_velocity_burst.sh 1'
    exit 0
fi
