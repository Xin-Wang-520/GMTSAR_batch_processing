...
Usage:
  ...
  ./run4.2_generate_sbas_tables_pin_burst.sh 1
...
Outputs:
  sbas_burst_pin/intf.tab
  sbas_burst_pin/scene.tab
  sbas_burst_pin/prep_sbas.log
  sbas_burst_pin/range_check.log
  sbas_burst_pin/run_sbas_parallel.sh
  sbas_burst_pin/run4.2_complete

The generated SBAS command retains the conventional -rms -dem options.
Here -dem estimates residual DEM error after the external Run 3.10 correction.
...
if [[ -z "$WAVELENGTH" ]]; then
    ...
    WAVELENGTH_SOURCE='sbas_burst_pin/supermaster.PRM'
fi
...
printf 'Pair file check        : all unwrap_dem_correct_pin_up.grd/corr.grd exist and are non-empty\n'
...
printf 'Smoothing              : %s\n' "$SMOOTH"
printf 'SBAS internal DEM term : enabled (-dem; residual DEM error)\n'
printf '%s\n' '========================================'

if (( $# == 0 )); then
    usage
    printf '%s\n' '[CHECK ONLY] prep_sbas.csh and sbas_parallel were not run.'
    printf '%s\n' '[NEXT] ./run4.2_generate_sbas_tables_pin_burst.sh 1'
    exit 0
fi
...

{
    ...
    printf 'internal_dem_term=enabled_residual_after_external_correction\n'
    printf 'command=%s\n' "$SBAS_CMD"
} > run4.2_complete
