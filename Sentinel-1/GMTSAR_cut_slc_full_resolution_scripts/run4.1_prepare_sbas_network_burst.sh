...
# External DEM-error correction and reference-point correction are skipped.
...
usage() {
    cat <<'EOF'
========================================
Run 4.1: prepare the burst SBAS network (uncorrected unwrap)
...
No arguments:
  Check that all finalized Run 3.8 products exist and preview the SBAS network.
...
Run 3.9 must crop corr.grd to the exact unwrap.grd geometry first.
...
This workflow intentionally skips:
  unwrap_dem_correct_pin_up.grd
  external DEM-error correction
  reference-point/pin correction
...
while IFS=$'\t' read -r source_pair pair pair_log extra; do
    ...
    for name in corr.grd phasefilt.grd unwrap.grd unwrap.pdf; do
...
printf 'Complete unwrap.grd      : %s\n' "$PAIR_COUNT"
printf 'Non-empty corr.grd       : %s\n' "$CORR_PRESENT"
printf 'Unique acquisition scenes: %s\n' "$SCENE_LINES"
printf 'Unwrap source            : burst/intf_all/<pair>/unwrap.grd\n'
printf 'External DEM correction  : disabled\n'
printf 'Reference-pin correction : disabled\n'
...
if (( $# == 0 )); then
    ...
    printf '%s\n' '[NEXT] ./run4.1_prepare_sbas_network_burst.sh 1'
    exit 0
fi
...

{
    ...
    printf 'pair_root=%s\n' "$PAIR_ROOT"
    printf 'unwrap_name=unwrap.grd\n'
    printf 'corr_name=corr.grd\n'
    printf 'external_dem_correction=disabled\n'
    printf 'reference_pin_correction=disabled\n'
...
printf '%s\n' 'Run 4.1 did not move, rename or modify corr.grd or unwrap.grd.'
printf '%s\n' '========================================'
