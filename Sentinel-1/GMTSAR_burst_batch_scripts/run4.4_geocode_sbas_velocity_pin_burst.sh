...
FILTER_METERS="${SBAS_FILTER_METERS:-}"
CPT_MIN="${VEL_CPT_MIN:-}"
CPT_MAX="${VEL_CPT_MAX:-}"
...
Usage:
  ...
  ./run4.4_geocode_sbas_velocity_pin_burst.sh 1 [CPT_MIN CPT_MAX [CPT_STEP]]
...
Default settings:
  Spatial filter: automatically detected from gauss_<meters>
  Color range  : automatically read from the generated vel_ll.grd
  Example fixed : ./run4.4_geocode_sbas_velocity_pin_burst.sh 1 -20 20
  With step    : ./run4.4_geocode_sbas_velocity_pin_burst.sh 1 -20 20 2

Optional environment variables:
  SBAS_FILTER_METERS=400  (optional manual override)
...
Inputs:
  ...
  burst/intf_all/<pair>/gauss_<meters>
...
case $# in
    ...
    4)
        ...
        ;;
    *)
        die "usage: $0, or $0 1 [CPT_MIN CPT_MAX [CPT_STEP]]"
        ;;
esac

if [[ -n "$FILTER_METERS" ]]; then
...
if [[ -n "$CPT_MIN" || -n "$CPT_MAX" ]]; then
    ...
    CPT_MODE="fixed by $CPT_INPUT_SOURCE"
else
...
if [[ -n "$FILTER_METERS" ]]; then
    ...
    FILTER_SELECTION='forced by SBAS_FILTER_METERS'
    ...
    while IFS= read -r pair; do
        [[ -n "$pair" ]] || continue
        for candidate in "$PAIR_ROOT/$pair"/gauss_*; do
            [[ -s "$candidate" ]] || continue
            candidate_name="$(basename -- "$candidate")"
            if [[ "$candidate_name" =~ ^gauss_([1-9][0-9]*)$ ]]; then
                GAUSS_NAME="$candidate_name"
                FILTER_METERS="${BASH_REMATCH[1]}"
                GAUSS_SOURCE="$candidate"
                FILTER_SELECTION='automatically detected from interferogram'
                break 2
            fi
        done
    done < "$INTFLIST"
    [[ -n "$GAUSS_SOURCE" ]] ||
        die "cannot auto-detect a non-empty gauss_<meters> file in finalized burst pairs"
fi
...
backup_count=0
for name in trans.dat "$GAUSS_NAME" vel.pdf vel.png vel_ll.grd vel_ll.cpt vel_ll.pdf \
...
printf '%s\n' '[STEP 3] Plot radar-coordinate vel.grd as PDF and PNG'
gmt begin vel pdf,png
    gmt set \
...
gmt end
[[ -s vel.pdf ]] || die "vel.pdf was not generated"
[[ -s vel.png ]] || die "vel.png was not generated"

printf '%s\n' '[STEP 4] Plot the geographic velocity PDF'
...
printf '%s\n' '[STEP 5] Generate KML/KMZ products'
...

{
    ...
    printf 'filter_selection=%s\n' "$FILTER_SELECTION"
    printf 'gauss_source=%s\n' "$GAUSS_SOURCE"
    printf 'cpt_mode=%s\n' "$CPT_MODE"
    printf 'cpt_range=%s/%s/%s\n' "$CPT_MIN" "$CPT_MAX" "$CPT_STEP"
    printf 'velocity_radar=%s/vel.grd\n' "$(pwd -P)"
    printf 'radar_pdf=%s/vel.pdf\n' "$(pwd -P)"
    printf 'radar_png=%s/vel.png\n' "$(pwd -P)"
...
printf 'Radar grid    : %s/vel.grd\n' "$(pwd -P)"
printf 'Radar PDF     : %s/vel.pdf\n' "$(pwd -P)"
printf 'Radar PNG     : %s/vel.png\n' "$(pwd -P)"
