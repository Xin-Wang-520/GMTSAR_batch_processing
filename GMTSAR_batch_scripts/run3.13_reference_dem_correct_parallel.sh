...
usage() {
cat <<'USAGE'
Run 3.13: reference DEM-corrected interferograms
...
Output: merge/20*_*/unwrap_dem_correct_pin_up.grd
USAGE
}

ROOT="$(pwd -P)"
MERGE_DIR="$ROOT/merge"
[[ -d "$MERGE_DIR" ]] || { echo "[ERR] Run in a track directory containing merge/."; exit 1; }
cd "$MERGE_DIR"
mapfile -t DIRS < <(find . -maxdepth 1 -mindepth 1 -type d -name '20*_*' -printf '%f\n' | sort)
((${#DIRS[@]} > 0)) || { echo "[ERR] No merge/20*_* directories."; exit 1; }

MISSING=run3.13_missing_inputs.tsv
: > "$MISSING"
for d in "${DIRS[@]}"; do
...
echo "========================================"
echo "Run 3.13 input check"
echo "Track root           : $ROOT"
echo "Interferogram pairs  : ${#DIRS[@]}"
echo "Valid corrected grids: $((${#DIRS[@]}-N_MISSING))/${#DIRS[@]}"
echo "========================================"
if [[ -s "$MISSING" ]]; then echo "[ERR] See $MERGE_DIR/$MISSING"; cat "$MISSING"; exit 1; fi
rm -f "$MISSING"
...
RUN_LIST=run3.13_reference_pending.txt
FAILED=run3.13_reference_failed.tsv
RESULTS=run3.13_reference_values.tsv
: > "$RUN_LIST"
: > "$FAILED"
for d in "${DIRS[@]}"; do
...
run_one() {
  ...
  (
    cd "$d"
    tmp=$(mktemp .run3.13.XXXXXX.grd)
    trap 'rm -f "$tmp"' EXIT INT TERM
    gmt grdcut unwrap_dem_correct.grd -R"$region" -G"$tmp"
    median=$(gmt grdinfo "$tmp" -L2 -C | awk '{print $12}')
...
if [[ -s "$FAILED" || $status -ne 0 ]]; then echo "[ERROR] See $MERGE_DIR/$FAILED"; exit 1; fi
rm -f "$RUN_LIST" "$FAILED"
printf 'region=%s\npairs=%s\ncompleted=%s\n' "$REGION" "${#DIRS[@]}" "$(date -Is)" > run3.13_complete
echo "========================================"
echo "[DONE] Reference correction: ${#DIRS[@]}/${#DIRS[@]} passed validation."
echo "Output list: $MERGE_DIR/$RESULTS"
echo "========================================"
