...
#
# Standalone Run 3.6 variant:
# use SAT_llt2rat_para with OpenMP when a new trans.dat is required.
...
FRAMES=(F1 F2 F3)
PARALLEL_LLT2RAT="${SAT_LLT2RAT_PARA_BIN:-/home/xinw/bin/own/SAT_llt2rat_para}"
...
usage() {
    cat <<'EOF'
Usage:
  ./run3.6_merge_F123_parallel_trans.sh
  ./run3.6_merge_F123_parallel_trans.sh 1 MERGE_JOBS
  ./run3.6_merge_F123_parallel_trans.sh 2 MERGE_JOBS TRANS_THREADS
...
Mode 2 (full merge):
  ...
  directories, generate trans.dat with SAT_llt2rat_para when needed,
  and merge all accepted interferogram pairs.
...
TRANS_THREADS:
  Number of OpenMP threads used only while generating trans.dat.
  Recommended: 5. Reduce it if CPU or memory is limited.
EOF
}
...
show_guide() {
    cat <<'EOF'
========================================
Run 3.6 parallel-trans command guide (processing NOT started)

Choose one formal-merge implementation:

Standard GMTSAR SAT_llt2rat:
  ./run3.6_merge_F123.sh 2 15

Parallel SAT_llt2rat_para:
  ./run3.6_merge_F123_parallel_trans.sh 2 15 5

Both scripts perform the same checks, seam preview and F1/F2/F3 merge.
Only the trans.dat generator is different.

Step 1 - merge the first, middle and last merge_list records and plot them:
  ./run3.6_merge_F123_parallel_trans.sh 1 15

Step 2 - after inspecting the plots, generate trans.dat in parallel when
needed and merge all pairs:
  ./run3.6_merge_F123_parallel_trans.sh 2 15 5

Parallel setting:
  15 = merge at most fifteen interferogram pairs concurrently (default)
  5  = use five OpenMP threads to generate trans.dat
...
Long full-merge server run (after mode 1 is accepted):
  nohup ./run3.6_merge_F123_parallel_trans.sh 2 15 5 \
    > run3.6_merge_F123_parallel_trans.nohup.log 2>&1 &

Monitor:
  tail -f run3.6_merge_F123_parallel_trans.nohup.log
  tail -f merge/merge_batch.log

Parallel trans.dat dependency:
  /home/xinw/bin/own/SAT_llt2rat_para

trans.dat validation:
  A file larger than 20 MiB is reused.
  A missing, empty or <= 20 MiB file is removed and generated again.

This is a standalone script. It does not call run3.6_merge_F123.sh.
...
confirm_and_delete_missing_pairs() {
    ...
    if [[ ! -t 0 ]]; then
        die "interactive deletion approval is required; rerun './run3.6_merge_F123_parallel_trans.sh 1 15' in a terminal"
    fi
...
MODE="${1:-}"
[[ "${MODE}" == "1" || "${MODE}" == "2" ]] ||
    die "MODE must be 1 (preview) or 2 (full merge)"

if [[ "${MODE}" == "1" ]]; then
    (( $# == 2 )) ||
        die "mode 1 usage: ./run3.6_merge_F123_parallel_trans.sh 1 MERGE_JOBS"
    MERGE_JOBS="$2"
    TRANS_THREADS=0
else
    (( $# == 3 )) ||
        die "mode 2 usage: ./run3.6_merge_F123_parallel_trans.sh 2 MERGE_JOBS TRANS_THREADS"
    MERGE_JOBS="$2"
    TRANS_THREADS="$3"
    [[ "${TRANS_THREADS}" =~ ^[1-9][0-9]*$ ]] ||
        die "TRANS_THREADS must be a positive integer: ${TRANS_THREADS}"
    [[ -x "${PARALLEL_LLT2RAT}" ]] ||
        die "parallel executable not found or not executable: ${PARALLEL_LLT2RAT}"
fi
...
if [[ "${MODE}" == "2" ]]; then
    printf 'trans.dat threads      : %s\n' "${TRANS_THREADS}"
    printf 'Parallel converter     : %s\n' "${PARALLEL_LLT2RAT}"
fi
...
setup_parallel_trans_override() {
    OVERRIDE_BIN="${ROOT_DIR}/merge/.run3.6_parallel_trans_bin"
    mkdir -p "${OVERRIDE_BIN}"

    cat > "${OVERRIDE_BIN}/SAT_llt2rat" <<'EOF_PARALLEL_WRAPPER'
#!/usr/bin/env bash
set -euo pipefail

if (( $# < 2 || $# > 3 )); then
    printf '[PARALLEL TRANS ERROR] Unexpected SAT_llt2rat arguments:' >&2
    printf ' %q' "$@" >&2
    printf '\n' >&2
    exit 2
fi
...
OUTPUT_MODE="${3:-}"
[[ -f "${PRM_FILE}" ]] || {
    printf '[PARALLEL TRANS ERROR] PRM not found: %s\n' "${PRM_FILE}" >&2
    exit 1
}
[[ -f dem.grd ]] || {
    printf '%s\n' '[PARALLEL TRANS ERROR] dem.grd not found in current merge directory' >&2
    exit 1
}

printf '[PARALLEL TRANS] %s threads, PRM=%s, DEM=dem.grd\n' \
    "${OMP_NUM_THREADS:-runtime-default}" "${PRM_FILE}" >&2

# GMTSAR pipes `gmt grd2xyz` into SAT_llt2rat. The parallel executable
# reads dem.grd directly, so close stdin and translate the command:
#   SAT_llt2rat master.PRM 1 -bod
# to:
#   SAT_llt2rat_para master.PRM dem.grd -bod
exec </dev/null
if [[ -n "${OUTPUT_MODE}" ]]; then
    exec "${SAT_LLT2RAT_PARA_BIN}" "${PRM_FILE}" dem.grd "${OUTPUT_MODE}"
else
    exec "${SAT_LLT2RAT_PARA_BIN}" "${PRM_FILE}" dem.grd
fi
EOF_PARALLEL_WRAPPER

    chmod +x "${OVERRIDE_BIN}/SAT_llt2rat"
    export SAT_LLT2RAT_PARA_BIN="${PARALLEL_LLT2RAT}"
    export OMP_NUM_THREADS="${TRANS_THREADS}"
    export PATH="${OVERRIDE_BIN}:${PATH}"

    printf '%s\n' '[PARALLEL TRANS] Temporary SAT_llt2rat override enabled.'
    printf '[PARALLEL TRANS] OpenMP threads: %s\n' "${TRANS_THREADS}"
    printf '[PARALLEL TRANS] Executable: %s\n' "${PARALLEL_LLT2RAT}"
}
...
[[ -s merge/run3.6_preview_complete ]] ||
    die "mode-1 preview is not complete; run './run3.6_merge_F123_parallel_trans.sh 1 15' first"
...
if (( TRANS_SIZE_BYTES > TRANS_MIN_BYTES )); then
    printf '[PARALLEL TRANS] Existing merge/trans.dat is %.2f MiB (> 20 MiB); reuse it.\n' \
        "$(awk -v bytes="${TRANS_SIZE_BYTES}" 'BEGIN {print bytes / 1024 / 1024}')"
else
    if [[ -e merge/trans.dat || -L merge/trans.dat ]]; then
        printf '[PARALLEL TRANS] Existing merge/trans.dat is %.2f MiB (<= 20 MiB); remove and regenerate it.\n' \
            "$(awk -v bytes="${TRANS_SIZE_BYTES}" 'BEGIN {print bytes / 1024 / 1024}')"
        rm -f -- merge/trans.dat
    else
        printf '%s\n' '[PARALLEL TRANS] merge/trans.dat is absent; generate it now.'
    fi
    setup_parallel_trans_override
fi
