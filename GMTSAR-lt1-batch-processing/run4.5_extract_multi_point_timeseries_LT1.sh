#!/usr/bin/env bash
# Run 4.5: extract LT-1 SBAS displacement time series at multiple geographic
...
usage() {
    cat <<'EOF'
========================================
Run 4.5: extract multi-point LT-1 SBAS time series

Usage:
  ./run4.5_extract_multi_point_timeseries_LT1.sh [POINTS_FILE] [WINDOW_X WINDOW_Y]
  ./run4.5_extract_multi_point_timeseries_LT1.sh LONGITUDE LATITUDE LABEL [WINDOW_X WINDOW_Y]
...
This script always reads sbas_detrend; the default sampling window is 5 x 5 nodes.
...
Examples:
  ...
  ./run4.5_extract_multi_point_timeseries_LT1.sh my_points.txt
  ./run4.5_extract_multi_point_timeseries_LT1.sh my_points.txt 3 3
  ./run4.5_extract_multi_point_timeseries_LT1.sh 118.781 29.993 luoshixing
  ./run4.5_extract_multi_point_timeseries_LT1.sh 118.781 29.993 luoshixing 3 3

Optional event marker:
  LANDSLIDE_DATE=2025-05-25 LANDSLIDE_LABEL=Landslide ./run4.5_extract_multi_point_timeseries_LT1.sh ...

Inputs:
  topo/dem.grd
  SLC/LT1_<master>.PRM and SLC/LT1_<master>.LED
  sbas_detrend/scene.tab
  sbas_detrend/disp_<YYYYDDD>.grd
  sbas_detrend/vel_ll.grd

Outputs in sbas_detrend/run4.5_time_series/:
  points_radar.tsv
  time_series_all.tsv
...
RUN42="$SBAS_DIR/run4.2_complete"

[[ -s "$RUN42" ]] || die "missing or empty: $RUN42; complete Run 4.2 first"
MASTER_STEM="$(awk -F= '$1=="master"{print $2; exit}' "$RUN42")"
[[ "$MASTER_STEM" =~ ^LT1_[0-9]{8}$ ]] ||
    die "cannot read a valid LT-1 master from $RUN42"
PRM="$ROOT/SLC/${MASTER_STEM}.PRM"
LED="$ROOT/SLC/${MASTER_STEM}.LED"

for file in "$DEM" "$PRM" "$LED" "$SCENE_TAB" "$VEL_LL"; do
    [[ -s "$file" ]] || die "missing or empty: $file"
done

TMP="$(mktemp -d "${TMPDIR:-/tmp}/run4.5-lt1.XXXXXX")"
trap 'rm -rf -- "$TMP"' EXIT INT TERM
GMT_USERDIR="$TMP/gmt_user"
export GMT_USERDIR
mkdir -p "$GMT_USERDIR"

# Prepare a private PRM/LED pair for SAT_llt2rat without changing SLC/.
SAT_WORK="$TMP/sat_llt2rat"
SAT_PRM="$SAT_WORK/${MASTER_STEM}.PRM"
mkdir -p "$SAT_WORK"
if ! awk -v led="${MASTER_STEM}.LED" '
    $1 == "led_file" {
        print "led_file = " led
        found=1
        next
...
' "$PRM" > "$SAT_PRM"; then
    die "master PRM contains no led_file field: $PRM"
fi
[[ -s "$SAT_PRM" ]] || die "failed to prepare the temporary master PRM"
ln -s "$LED" "$SAT_WORK/${MASTER_STEM}.LED"
...
while IFS= read -r scene; do
    grid="$SBAS_DIR/disp_${scene}.grd"
    signature="$(gmt grdinfo "$grid" -C | awk 'NR==1 {
        print $2,$3,$4,$5,$8,$9,$10,$11,$12
    }')"
    [[ "$signature" == "$GRID_SIGNATURE" ]] ||
        die "disp_${scene}.grd geometry differs from disp_${FIRST_SCENE}.grd"
done < "$SCENES"

# The LT-1 cropped master PRM, interferograms and SBAS displacement grids retain
# the same radar-coordinate system. No burst-style region_cut offset is needed.
REGION_CUT='not_applicable_LT1_native_coordinates'
RADAR_OFFSET_RANGE=0
RADAR_OFFSET_AZIMUTH=0

printf '%s\n' '========================================'
printf '%s\n' 'Run 4.5 multi-point LT-1 SBAS time-series check'
printf 'Track root        : %s\n' "$ROOT"
printf 'SBAS branch       : %s\n' "$BRANCH"
printf 'SBAS directory    : %s\n' "$SBAS_DIR"
printf 'Master PRM/LED    : %s\n' "$MASTER_STEM"
...
printf 'Radar grid        : %s\n' "$GRID_SIGNATURE"
printf 'Radar coordinates : native cropped LT-1 coordinates (no origin shift)\n'
...
while IFS=$'\t' read -r label lon lat; do
    ...
    radar_range="$(awk -v value="$full_radar_range" 'BEGIN {printf "%.0f",value}')"
    radar_azimuth="$(awk -v value="$full_radar_azimuth" 'BEGIN {printf "%.0f",value}')"
    ...
    awk -v x="$radar_range" -v y="$radar_azimuth" \
        ...
        die "$label radar coordinate ($radar_range,$radar_azimuth) is outside the SBAS displacement grids"

    nearest_range="$(awk -v x="$radar_range" -v xmin="$XMIN" -v dx="$XINC" -v reg="$REGISTRATION" \
        'BEGIN {x0=xmin+(reg==1?dx/2:0); printf "%.12g",x0+int((x-x0)/dx+0.5)*dx}')"
    nearest_azimuth="$(awk -v y="$radar_azimuth" -v ymin="$YMIN" -v dy="$YINC" -v reg="$REGISTRATION" \
        'BEGIN {y0=ymin+(reg==1?dy/2:0); printf "%.12g",y0+int((y-y0)/dy+0.5)*dy}')"
...
gmt begin "$OUTPUT_DIR/time_series_all" pdf,png
    ...
    gmt basemap -R"$DATE_MIN/$DATE_MAX/$GLOBAL_YMIN/$GLOBAL_YMAX" -JX16c/9c \
        -Bxa6Of3O+l"Date" -Byaf+l"LOS displacement (mm)" \
        -BWSen+t"LT-1 SBAS displacement time series"
...
gmt begin "$OUTPUT_DIR/vel_ll_points" pdf,png
    ...
    gmt grdimage "$VEL_LL" -JM15c -C"$MAP_CPT" \
        -Baf -BWSen+t"LT-1 SBAS velocity and Run 4.5 points"
