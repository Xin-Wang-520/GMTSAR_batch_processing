...
Usage:
  ...
  ./run3.8_stack_coherence_mask_LT1_parallel.sh THRESHOLD [BATCH] [JOBS]

No arguments:
  Print this guide and check LT-1 corr.grd completeness.
...
Processing:
  1. Confirm every intf_all/20* pair has a non-empty corr.grd.
  2. Regenerate intf_all/grid_list from the validated LT-1 products.
...
require_track_root() {
    ...
    root="$(pwd -P)"
    track="$(basename -- "${root}")"
    [[ -d intf_all ]] || die "cannot find intf_all/"
}
...
make_pair_list() {
    ...
    while IFS= read -r -d '' path; do
        ...
        printf '%s\n' "${pair}" >> "${output_file}"
    done < <(find intf_all -mindepth 1 -maxdepth 1 -type d -name "${FRAMES_PATTERN}" -print0)

    sort -u -o "${output_file}" "${output_file}"
    [[ -s "${output_file}" ]] ||
        die "no LT-1 interferogram directories found under intf_all/"
}
...
check_corr_inputs() {
    ...
    while IFS= read -r pair; do
        ...
        if [[ -s "intf_all/${pair}/corr.grd" ]]; then
            # Store paths relative to intf_all/, where GMT processing runs.
            printf '%s/corr.grd\n' "${pair}" >> "${corr_list}"
    ...
    printf '%s\n' '========================================'
    printf '%s\n' 'Run 3.8 LT-1-coherence input check'
    printf 'LT-1 pair directories : %d\n' "${pair_count}"
    ...
    printf '[CHECK OK] All %d LT-1 pairs contain a non-empty corr.grd.\n' \
        "${pair_count}"
}
...
(( $# >= 1 && $# <= 3 )) ||
    die "formal usage: ./run3.8_stack_coherence_mask_LT1_parallel.sh THRESHOLD [BATCH] [JOBS]"
THRESHOLD="$1"
...
cd intf_all
# Full grids only: Run 3.7 ROI is not applied here.
reference_signature=""
while IFS= read -r grid; do
    signature="$(gmt grdinfo "$grid" -Cn | awk '{print $1,$2,$3,$4,$7,$8,$9,$10,$11}')"
    [[ -n "$reference_signature" ]] || reference_signature="$signature"
    [[ "$signature" == "$reference_signature" ]] || die "grid geometry differs: $grid"
done < "${VALID_CORR_LIST}"
cp "${VALID_CORR_LIST}" "${LIST_NAME}"
CT="$(wc -l < "${LIST_NAME}" | awk '{print $1}')"
(( CT > 0 )) || die "generated list is empty: intf_all/${LIST_NAME}"
FIRST="$(head -n 1 "${LIST_NAME}")"
[[ -s "${FIRST}" ]] || die "first corr grid is missing or empty: intf_all/${FIRST}"
...
printf 'Threshold        : %s\n' "${THRESHOLD}"
printf 'Mean coherence   : %s/intf_all/mean_corr.grd\n' "${ROOT_DIR}"
printf 'Mask grid        : %s/intf_all/mask_def.grd\n' "${ROOT_DIR}"
printf 'Mask figure      : %s/intf_all/mask_def.pdf\n' "${ROOT_DIR}"
printf '%s\n' '========================================'
