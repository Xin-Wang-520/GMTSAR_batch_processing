...
#
# Run 3.10: validated, resumable parallel unwrapping for intf_all/20* pairs.
...
PID_FILE="intf_all/run3.10_unwrap.pid"
FAILED_REPORT="intf_all/run3.10_failed_pairs.tsv"
MISSING_INPUT_REPORT="intf_all/run3.10_missing_inputs.tsv"
LOG_DIR_NAME="run3.10_unwrap_logs"
NOHUP_LOG_NAME="run3.10_unwrap_LT1_parallel.nohup.log"
...
usage() {
    cat <<'EOF'
Usage:
  ./run3.10_unwrap_LT1_parallel.sh
  ./run3.10_unwrap_LT1_parallel.sh 1 [CORR_THRESHOLD] [PAIR]
  ./run3.10_unwrap_LT1_parallel.sh 2 JOBS [CORR_THRESHOLD]

No arguments:
  Check all inputs and existing unwrap outputs, then print this guide.
  No processing is started and no files under intf_all/ are modified.

Mode 1 - preview one pre-SNAPHU input:
  ./run3.10_unwrap_LT1_parallel.sh 1
  ./run3.10_unwrap_LT1_parallel.sh 1 0.0001
  ./run3.10_unwrap_LT1_parallel.sh 1 0.0001 2021051_2021063

  CORR_THRESHOLD    Correlation threshold; default: 0.0001.
  PAIR              Exact intf_all/<pair>/ directory name. If omitted, the
                    middle pair in the sorted interferogram list is selected.

  Applied valid-pixel inputs:
    corr.grd >= CORR_THRESHOLD
    mask.grd
    intf_all/mask_def.grd
    intf_all/landmask_ra.grd

  Outputs:
    intf_all/run3.10_presnaphu_preview/<pair>_combined_mask_presnaphu.pdf
    intf_all/run3.10_presnaphu_preview/<pair>_phase_presnaphu.pdf
...
Region in all modes:
  Default: read run3.7_unwrap_roi/radar_region.txt when it exists.
  If that file does not exist, you must explicitly provide --region or --full.
  Override: append --region rng0/rng1/azi0/azi1 to any command.
  Full frame: append --full to use the complete phasefilt.grd radar region.
  Full input grids and masks stay unchanged; snaphu_interp.csh crops internally.
  A changed region/threshold or changed shared masks invalidates resume markers.

Formal processing:
  1. Check intf_all/landmask_ra.grd and intf_all/mask_def.grd.
  2. Check every intf_all/20* directory for non-empty corr.grd, mask.grd and
     phasefilt.grd.
  3. Confirm the two common masks match the phasefilt.grd radar grid.
  4. Skip only validated outputs with a matching Run 3.10 completion marker.
...
Outputs in every intf_all/<pair>/:
  unwrap.grd
  unwrap.pdf
  conncomp.grd
...
Monitor mode 2:
  ...
  tail -f intf_all/run3.10_unwrap_logs/<pair>.log
EOF
}
...
make_pair_list() {
    ...
    [[ -s "${output_file}" ]] || die "no LT-1 interferogram directories found under ${MERGE_DIR}/"
}
...
check_inputs_and_outputs() {
    ...
    # Require the requested ROI to be inside and aligned with the full grid.
    gmt grdinfo "${TEMPLATE_GRID}" -Cn | awk -v reg="$UNWRAP_REGION" '
        BEGIN {split(reg,r,"/")} {
            if(r[1]<$1 || r[2]>$2 || r[3]<$3 || r[4]>$4)exit 1;
            for(i=1;i<=4;i++) {base=(i<=2?$1:$3); inc=(i<=2?$7:$8); q=(r[i]-base)/inc; if((q-int(q+0.5))^2>1e-10)exit 1}
        }' || die "ROI outside the phase grid or not aligned to grid boundaries; rerun Run 3.7"
    MASK_KEY="$(cksum "${MERGE_DIR}/mask_def.grd" "${MERGE_DIR}/landmask_ra.grd" | awk '{printf "%s-%s;",$1,$2}')"
    RUN_KEY="$UNWRAP_REGION|${CORR_THRESHOLD:-$PREVIEW_THRESHOLD}|${MAX_DISCONTINUITY:-$DEFAULT_MAX_DISCONTINUITY}|$MASK_KEY"
    while IFS= read -r pair; do
        ...
        for grid in corr.grd mask.grd phasefilt.grd; do
            if [[ -s "${MERGE_DIR}/${pair}/${grid}" ]]; then
                [[ "$(grid_signature "${MERGE_DIR}/${pair}/${grid}")" == "$TEMPLATE_SIGNATURE" ]] || die "grid geometry mismatch: $pair/$grid"
                case "${grid}" in
        ...
        if [[ -s "${MERGE_DIR}/${pair}/unwrap.grd" && -s "${MERGE_DIR}/${pair}/unwrap.pdf" && -s "${MERGE_DIR}/${pair}/run3.10_complete.txt" ]] && [[ "$(cat "${MERGE_DIR}/${pair}/run3.10_complete.txt")" == "$RUN_KEY" ]] && unwrap_geometry_ok "${MERGE_DIR}/${pair}/unwrap.grd"; then
...
make_presnaphu_preview() {
    ...
    gmt begin "${mask_output_base}" pdf
        ...
        gmt grdimage "${mask_all}" -R"$UNWRAP_REGION" -JX7i -C"${mask_cpt}" \
    ...
    gmt begin "${phase_output_base}" pdf
        gmt set MAP_FRAME_TYPE plain FONT_ANNOT_PRIMARY 10p \
            FONT_LABEL 11p FONT_TITLE 13p COLOR_NAN gray
        gmt grdimage "${phase_preview}" -R"$UNWRAP_REGION" -JX7i -C"${phase_cpt}" \
            -Bxaf+l"Range" -Byaf+l"Azimuth" \
...
# Read the Run 3.7 region. --region may override it in any mode.
REGION_OVERRIDE=""
FORCE_FULL=0
POSITIONAL=()
while (( $# > 0 )); do
    case "$1" in
        --region)
            (( $# >= 2 )) || die "--region requires rng0/rng1/azi0/azi1"
            REGION_OVERRIDE="$2"
            shift 2
            ;;
        --full)
...
[[ -z "$REGION_OVERRIDE" || "$FORCE_FULL" -eq 0 ]] || die "do not use --region and --full together"

full_phase_region() {
    local template
    command -v gmt >/dev/null 2>&1 || die "gmt was not found in PATH"
    template="$(find intf_all -mindepth 2 -maxdepth 2 -type f -path 'intf_all/20*/phasefilt.grd' -print | LC_ALL=C sort | sed -n '1p')"
    [[ -n "$template" && -s "$template" ]] || die "cannot find intf_all/20*/phasefilt.grd for full-frame region"
    gmt grdinfo "$template" -Cn | awk 'NF>=4 {printf "%.12g/%.12g/%.12g/%.12g",$1,$2,$3,$4; exit}'
}

if [[ -n "$REGION_OVERRIDE" ]]; then
    UNWRAP_REGION="$REGION_OVERRIDE"
    REGION_SOURCE="command-line --region"
elif (( FORCE_FULL == 1 )); then
    UNWRAP_REGION="$(full_phase_region)"
    REGION_SOURCE="full phasefilt.grd (--full)"
elif [[ -s run3.7_unwrap_roi/radar_region.txt ]]; then
    UNWRAP_REGION="$(awk 'NF {print $1; exit}' run3.7_unwrap_roi/radar_region.txt)"
    REGION_SOURCE="run3.7_unwrap_roi/radar_region.txt"
else
    die "run3.7_unwrap_roi/radar_region.txt was not found; specify --region rng0/rng1/azi0/azi1 or explicitly choose --full"
fi
[[ "$UNWRAP_REGION" =~ ^[0-9]+([.][0-9]+)?/[0-9]+([.][0-9]+)?/[0-9]+([.][0-9]+)?/[0-9]+([.][0-9]+)?$ ]] || die "invalid radar region: $UNWRAP_REGION"
UNWRAP_REGION="$(awk -F/ '{if($1>=$2 || $3>=$4)exit 1;printf "%.12g/%.12g/%.12g/%.12g",$1,$2,$3,$4}' <<< "$UNWRAP_REGION")" || die "empty radar region"
echo "[REGION] $UNWRAP_REGION"
echo "[REGION SOURCE] $REGION_SOURCE"
echo "[REGION METHOD] Cropping is performed inside snaphu_interp.csh"

unwrap_geometry_ok() {
    local grid="$1"
    gmt grdinfo "$grid" -Cn | awk -v reg="$UNWRAP_REGION" '
        BEGIN {split(reg,r,"/")} {for(i=1;i<=4;i++) if(($i-r[i])^2>1e-12)exit 1}'
}
...
if [[ "${1:-}" == '--internal-run' ]]; then
    ...
    case "$1" in
        1)
            ...
            shift
            (( $# <= 2 )) ||
                die "mode 1 usage: ./run3.10_unwrap_LT1_parallel.sh 1 [CORR_THRESHOLD] [PAIR]"
        ...
        2)
            shift
            (( $# >= 1 && $# <= 2 )) ||
                die "mode 2 usage: ./run3.10_unwrap_LT1_parallel.sh 2 JOBS [CORR_THRESHOLD]"
            JOBS="$1"
            ...
            [[ -s "${MERGE_DIR}/mask_def.grd" && -s "${MERGE_DIR}/landmask_ra.grd" ]] || die "complete Run 3.8 and Run 3.9 first"
            ROOT_DIR="$(pwd -P)"
            ...
            nohup "${SCRIPT_PATH}" --internal-run \
                "${JOBS}" "${CORR_THRESHOLD}" "${MAX_DISCONTINUITY}" --region "${UNWRAP_REGION}" \
...
rm -f unwrap.grd unwrap.pdf conncomp.grd phasefilt_interp.grd run3.10_complete.txt
snaphu_interp.csh ${CORR_THRESHOLD} ${MAX_DISCONTINUITY} ${UNWRAP_REGION} >& \$pair_log
set unwrap_status = \$status
if (\$unwrap_status == 0 && -s unwrap.grd && -s unwrap.pdf) then
    echo "${RUN_KEY}" > run3.10_complete.txt
endif
...
while IFS= read -r pair; do
    ...
    if [[ -z "$missing_csv" ]]; then
        if [[ ! -s "${pair}/run3.10_complete.txt" ]] || [[ "$(cat "${pair}/run3.10_complete.txt")" != "$RUN_KEY" ]] || ! unwrap_geometry_ok "${pair}/unwrap.grd"; then
            missing_csv="completion_marker_or_region"
        fi
    fi
    if [[ -n "${missing_csv}" ]]; then
